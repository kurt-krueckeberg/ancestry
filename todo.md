# Plan

## Moving the `~/antora-genealogy/m/persons` files to `~/anc`

Convert the `~/doc/modules/bleeke/page/1853-school-enumeration.adoc` and
the `~/doc/modules/krueckeberg/pages/1853-school-enumeration` so that they
do use a table format that not result in their being converted by adoc2jb1
to raw html table placed in _table_artifacts. 

## Consolidate Research Ideas/Plan Here

- Move the files that are in `~/gen/m/research` to this site.

- Review and rewrite them.

See also this guidance on using Claude: <https://gemini.google.com/app/1c715ae73bf138b1>.

Read:

- [From Spreadsheet to Research Report: Using Claude’s Custom Skills for Genealogy](https://familylocket.com/from-spreadsheet-to-research-report-using-claudes-custom-skills-for-genealogy/)
- [Claude Projects for genealogy](https://www.youtube.com/watch?v=rMZnN7iaGNk)

## Move the Ancestry Pages to ancestry.krtueckeberg.org?

Move `~/gen/m/ancestry` to another subdomain.

## Read Thomas Jones Genealogyical Proof
